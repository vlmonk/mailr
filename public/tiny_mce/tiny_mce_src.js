...
TinyMCE.prototype.updateContent = function(form_element_name) {
	...
	for (var n in tinyMCE.instances) {
		var inst = tinyMCE.instances[n];
                if (typeof(inst) == "function") continue;
		inst.switchSettings();
...
TinyMCEControl.prototype.execCommand = function(command, user_interface, value) {
	...
	if (command != "mceAddUndoLevel" && command != "Undo" && command != "Redo" && command != "mceImage" && command != "mceLink" && command != "mceAttachment" && command != "mceToggleVisualAid" && (command != "mceInsertTable" && !user_interface))
		this.execCommand("mceAddUndoLevel");
	...
	switch (command) {
		...
		break;
                ...
                if (this.settings['insertlink_callback']) {
                    ...
                    tinyMCE.openWindow(this.insertAttachmentTemplate, {href : href, target : target, title : title, onclick : onclick, action : action});
                }
                break;
...
TinyMCEControl.prototype.onAdd = function(replace_element, form_element_name, target_document) {
	...
	if (eval("typeof(" + templateFunction + ")") != 'undefined')
        ...
        var templateFunction = tinyMCE._getThemeFunction('_getInsertAttachmentTemplate');
        if (eval("typeof(" + templateFunction + ")") != 'undefined')
            this.insertAttachmentTemplate = eval(templateFunction + '(this.settings);');
